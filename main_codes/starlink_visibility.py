...
10/05/23 --> working with passages that are already reduced to Starlinks only
...
if not os.path.exists(f"/net/beulakerwijde/data1/breslin/my_code/full_selection_pool"):
    os.makedirs(f"/net/beulakerwijde/data1/breslin/my_code/full_selection_pool")
...
def image_info(target):
    ...
    images = np.sort(glob.glob(f'{subtracted}{target}/diffimages/diff_*.fits.gz'))
...
for i, camid in enumerate(camids):
    ...
    all_passages.append(pickle.load(open(f"{subtracted}{target}/passages_{target}.p", "rb")))
...
for (fast, camid, pool) in zip(fastfiles, camids, pools):        
    ...
    if len(pool) != 0:
        ...
        pickle.dump(pool, open(f'{user_path}full_selection_pool/pool_{camid}.p', 'wb'))
