...
if not os.path.exists(f"{rootdir}{user}/data/subtracted/{target}"):
    ...
    os.makedirs(f"{rootdir}{user}/data/subtracted/{target}/diffimages")
...
def diffimages_fn(sss):
    ...
    print(f'Finished adding images: took {(time.time() - t0)/60} minutes')
    ...
    print(f'Passages took {(time.time() - t0)/60} minutes')
    ...
    for iii in range(nlong[sss]-1):
        ...
        filename = os.path.join(f"{rootdir}{user}/data/subtracted/{target}/diffimages/", filename)
    
        hdu = pf.PrimaryHDU(data=subimage[:,:,iii], header=diffheader)
        hdu.writeto(filename, overwrite=True)
    print(f'Difference images took {(time.time() - t0)/60} minutes')


print(f'Looping through {len(longsets)} batches')
for i, sss in enumerate(range(len(longsets))):
    print(f'Batch {i}:')
    diffimages_fn(sss)

print(f'Runtime: {(time.time()-t_start)/3600} hours')
