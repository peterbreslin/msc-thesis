...
Edited on Wed Jan 29 2020 by Thomas Wijnen (use camid instead of sitename to call read_siteinfo())
"""
...
def read_siteinfo(filename, camid):
    #CHANGED THIS FUNCTION BY USING CAMID INSTEAD OF SITENAME TO SELECT SITEINFO. | TW 29-1-2020
...
def read_catalogue(starcat=cfg.starcat):
    ...
    catalogue = np.rec.fromarrays([catalogue[key] for key in list(catalogue.keys())], names=[key for key in list(catalogue.keys())])    
