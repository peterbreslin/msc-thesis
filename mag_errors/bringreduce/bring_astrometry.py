...
#J2000 to equinox and vice versa routine copied from Remko Stuik | Thomas Wijnen 4-2-2020
...
def world2wcs(wcspars, ra, dec, lst=None, jd=None):
    ...
    #Added jd as extra parameter. Copied from Remko Stuik because data after 1-1-2018 could have an offset of ~6 pixels | Thomas Wijnen 4-2-20202
    #Only need to convert if JD is before 1/1/2018 (2457754.5)
    if jd is not None and jd > 2457754.5:
...
#Copied astromask routines from Remko Stuik.| Thomas Wijnen 4-2-2020
...
class Astrometry(object):
    ...
    def solve(self, image, header, ra, dec, drate=.6, maxiter=10, margin=668, ntries=1):
        ...
        """
        #Changed ['LST'] and ['LSTSEQ'] to all small ['lst'] and ['lstseq'], copied from Remko Stuik | Thomas Wijnen 4-2-2020
        ...
        # Set the equinox. Copied from Remko Stuik. Only necessary for data after 1/1/2018 (JD 2457754.5) | Thomas Wijnen 4-2-2020
        #if header['jd'] > 2457754.5:
        #removed the call to J2000_to_equinox below and passed header['jd'] as an extra parameter to the call in world2pix below instead
        #ra, dec = j2000_to_equinox(ra, dec, header['jd'])        
        
        # Evaluate the solution.
        xpix, ypix, mask = self.world2pix(header['lst'], ra, dec, header['jd'])
        ...
        #Copied from Remko Stuik | Thomas Wijnen 4-2-2020
        if dr > 2.:
 
            log.info('Final solution has dr > 2, terminating...')
            aflag += 4
