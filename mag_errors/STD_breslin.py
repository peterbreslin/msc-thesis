...
class LineSegments(object):
    def __init__(self, deltaimage, nx, ny, starcat, astrometry, target, curlstseq, midlst, midJD, 
        ...
        self.fast = h5py.File(f'/Users/peter/Projects/starlink_data/fast_{target}.hdf5','r')
    ...
    def determine_satvmag(self, x_min, y_min, x_max, y_max, curlstseq):
        ...
        # The background represents the noise characteristics of the region where no signal (e.g., stars or satellite trails) is present.
        # This provides an estimate of the random noise in the image. 
        # skymod = Scalar giving estimated mode of the sky values
        # skysig = sigma = a scalar giving standard deviation of the peak in the sky
        ...
        sigma_F: could define as std of F by collecting many measurements of F. But, doesn't take all sources into account.
            - e.g. photon noise. Could assume sigma_F is completely due to photon noise
            - could compute this by getting std of pixels surronding the satellite trail

        """

        # Uncertainty due to photon noise = sigma_F
        # Need to ensure x_min, x_max, y_min, y_max are 'correctly' defined for this procedure
        if x_min < x_max:
            x0 = x_min
            x1 = x_max
        else:
        ...
        if y_min < y_max:
            ...
            y0 = y_max
            y1 = y_min
        mean_pxls, median_pxls, sigma_F = aps.sigma_clipped_stats(np.fabs(self.deltaimage[y0:y1, x0:x1]))
        ...
        sigma_m = self.brightness_err(satflux, sigma_F, sigma_B)

        return satvmag, sigma_m
    ...
    def brightness_err(self, logF, sigma_F, sigma_B):
        F = 10**logF
    ...
    # def photon_noise_err(self, logFlux, sigma_satflux):
    #     # Estimating the uncertainty due to photon noise 
    #     sigma_F = 10**logFlux * np.log(10) * sigma_satflux
    #     return sigma_F


    # def brightness_err(self, sigma_logFlux, sigma_Bg):
    #     """
    #     sigma_C = sqrt((dC/dA * sigma_A)^2 + (dC/dB * sigma_B)^2)

    #     logFlux is variable A, Background is variable B, Brightness Magnitude is variable C

    #     sigma_Mag = sqrt((d_Mag/d_logFlux * sigma_logFlux)^2 + (d_Mag/d_Bg * sigma_Bg)^2)

    #     """
    #     dMag_dlogFlux = -2.5
    #     dMag_dBg = 1
    #     sigma_Mag = np.sqrt((dMag_dlogFlux * sigma_logFlux)**2 + (dMag_dBg * sigma_Bg)**2)
    #     return sigma_Mag
