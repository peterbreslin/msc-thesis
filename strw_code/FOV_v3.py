...
def image_info(target):
    ...
    imginfo = {}
    for img in images[750:800]: #taking a subset for testing purposes
...
for seq in fastfiles[0]['station']['lstseq'][:]: #LSTSEQs will be the same for each camid
    ...
    for data, timerange, camid, fast, starlinks, passages, reslist, resdict in zip(
        imginfo, timeranges, camids, fastfiles, tles, passgs, reslists, resdicts):
...
for (reslist, resdict, camid) in zip(reslists, resdicts, camids):
