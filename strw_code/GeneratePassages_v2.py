		...
		for i, (jd, LSTSEQ) in enumerate(zip(JDs, lstseq_array)):
			...
			try:
				...
				if sat.alt > 0:
					...
					# So we will add start, mid, and end (mid will be the end/start for the positive/negative segment): 
					# start: obs.date = jd - 2415020-0.5*exp_array[i]/86400 
					# mid:   obs.date = jd - 2415020+0.5*exp_array[i]/86400 
					...
					# Middle
					obs.date = jd - 2415020+0.5*exp_array[i]/86400 #now looking at mid-point of the full segment 
					sat.compute(obs)
					...
					if mask0 and mask1 and mask2:                    
						...
						satnum = tle[1][2:8]
						passages[seq][satnum] = {'start':{}, 'mid':{}, 'end':{}, 'JD': jd}
						...
						passages[seq][satnum]['mid']['jd']  = jd+0.5*exp_array[i]/86400
						passages[seq][satnum]['mid']['lst'] = lst_array[i]+0.5*exp_array[i]/3600
						passages[seq][satnum]['mid']['ra']  = ra1
						passages[seq][satnum]['mid']['dec'] = dec1
						passages[seq][satnum]['mid']['x'] = x1[0]
						passages[seq][satnum]['mid']['y'] = y1[0]

						passages[seq][satnum]['end']['jd']  = jd+1.5*exp_array[i]/86400
						passages[seq][satnum]['end']['lst'] = lst_array[i]+1.5*exp_array[i]/3600
						passages[seq][satnum]['end']['ra']  = ra2
						passages[seq][satnum]['end']['dec'] = dec2
						passages[seq][satnum]['end']['x'] = x2[0]
						passages[seq][satnum]['end']['y'] = y2[0]
