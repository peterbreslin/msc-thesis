...
passages file is reduced to only the satellites given for the given image (for each of JD0, JD1, and
midJD of that image).
...
Reduce passages file to selection pool:
 ...
 - X = selection pool (LSTSEQs with [JD0s, JD1s, midJDs] each with satnums)
 - Y = P reduced to JDs found in X
 - Z = Y reduced such that each JD has only the satnums found in X
...
for i, curlstseq in enumerate(lstseqs[0:2]):
    t0 = time.time()
